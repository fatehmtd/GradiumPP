...
curl_mime* buildMime(CURL* curl, const HttpRequest& request)
{
    ...
    for (const auto& file : request.multipart_files) {
        ...
        if (rc != CURLE_OK) {
            curl_mime_free(mime);
            throw std::runtime_error("[gradiumpp] curl_mime_filedata failed for " + file.file_path);
        }
    }
    return mime;
}
...
HttpResponse CurlHttpTransport::send(const HttpRequest& request)
{
    ...
    if (curlGlobalInitResult != CURLE_OK) {
        throw std::runtime_error("[gradiumpp] curl_global_init failed");
    }

    CURL* curl = curl_easy_init();
    if (curl == nullptr) {
        throw std::runtime_error("[gradiumpp] curl_easy_init failed");
    }
    ...
    if (rc != CURLE_OK) {
        throw std::runtime_error(std::string("[gradiumpp] curl perform failed: ")
