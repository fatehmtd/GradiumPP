...
static const lws_protocols kProtocols[] = {
    { "gradiumpp-ws", lwsCallback, 0, 65536, 0, nullptr, 0 },
...
void LwsWebSocketTransport::connect(const WebSocketConnectOptions& options)
{
    ...
    if (!_impl->_ctx) {
        throw std::runtime_error("[gradiumpp] lws_create_context failed");
    }
    ...
    if (!_impl->_wsi) {
        lws_context_destroy(_impl->_ctx);
        _impl->_ctx = nullptr;
        throw std::runtime_error("[gradiumpp] lws_client_connect_via_info failed");
    }
