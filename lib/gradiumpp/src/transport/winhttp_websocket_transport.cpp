...
ParsedWsUrl parseWsUrl(const std::string& url)
{
    ...
    if (url.size() > 6 && url.substr(0, 6) == "wss://") {
        ...
        throw std::runtime_error("[gradiumpp] Invalid WebSocket URL: " + url);
    }
...
void WinHttpWebSocketTransport::connect(const WebSocketConnectOptions& options)
{
    ...
    _impl->_session = WinHttpOpen(
        ...
        0);
    if (!_impl->_session) {
        throw std::runtime_error("[gradiumpp] WinHttpOpen failed: "
                                 + std::to_string(GetLastError()));
    }

    _impl->_connection = WinHttpConnect(_impl->_session, url.host.c_str(), url.port, 0);
    if (!_impl->_connection) {
        throw std::runtime_error("[gradiumpp] WinHttpConnect failed: "
                                 + std::to_string(GetLastError()));
    }
    ...
    if (!_impl->_request) {
        throw std::runtime_error("[gradiumpp] WinHttpOpenRequest failed: "
                                 + std::to_string(GetLastError()));
    }

    if (!WinHttpSetOption(_impl->_request, WINHTTP_OPTION_UPGRADE_TO_WEB_SOCKET, nullptr, 0)) {
        throw std::runtime_error("[gradiumpp] WINHTTP_OPTION_UPGRADE_TO_WEB_SOCKET failed: "
                                 + std::to_string(GetLastError()));
    }
    ...
    if (!WinHttpSendRequest(_impl->_request, WINHTTP_NO_ADDITIONAL_HEADERS, 0,
        ...
        throw std::runtime_error("[gradiumpp] WinHttpSendRequest failed: "
                                 + std::to_string(GetLastError()));
    }

    if (!WinHttpReceiveResponse(_impl->_request, nullptr)) {
        throw std::runtime_error("[gradiumpp] WinHttpReceiveResponse failed: "
                                 + std::to_string(GetLastError()));
    }

    _impl->_websocket = WinHttpWebSocketCompleteUpgrade(_impl->_request, 0);
    if (!_impl->_websocket) {
        throw std::runtime_error("[gradiumpp] WinHttpWebSocketCompleteUpgrade failed: "
                                 + std::to_string(GetLastError()));
    }

    _isOpen.store(true);

    // Background receive loop with fragment reassembly
        ...
        while (_isOpen.load()) {
            ...
            if (rc != ERROR_SUCCESS) {
                if (_isOpen.exchange(false)) {
                    ...
                    if (cb) {
                        cb("[gradiumpp] WebSocket receive error: " + std::to_string(rc));
                    }
                }
                return;
            }
            ...
            case WINHTTP_WEB_SOCKET_UTF8_MESSAGE_BUFFER_TYPE: {
                ...
                if (cb) { cb(std::string(message.begin(), message.end())); }
                message.clear();
                pending_binary = false;
                break;
            }
...
{
    if (!_isOpen.load()) {
        throw std::runtime_error("[gradiumpp] WebSocket is not open");
    }
    ...
    if (rc != ERROR_SUCCESS) {
        throw std::runtime_error("[gradiumpp] WinHttpWebSocketSend failed: " + std::to_string(rc));
    }
}
